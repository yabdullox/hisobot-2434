...
async def start_report(msg: types.Message):
    ...
    await msg.answer("🖋 Hisobot matnini yuboring:", reply_markup=ReplyKeyboardRemove())
...
async def handle_report(msg: types.Message):
    ...
    if state == "waiting_report_text":
        ...
        return await msg.answer(f"✅ Hisobot tayyor:\n🧾 {worker_data[uid]['text']}\n💵 {summa:,} so‘m\n\n"
                                f"Tasdiqlash uchun 'tasdiqlash', bekor qilish uchun 'bekor' yozing.")
...
async def start_work(msg: types.Message):
    ...
    txt = f"✅ Ishni boshladingiz.\n🕒 {now.strftime('%H:%M')}"
    if bonus:
        txt += f"\n🎉 Bonus: +{bonus:,} so‘m"
    if fine:
        txt += f"\n⚠️ Jarima: -{fine:,} so‘m"

    await msg.answer(txt, reply_markup=worker_menu())
...
@router.message()
async def handle_end(msg: types.Message):
    uid = msg.from_user.id
...
async def clean_photo(msg: types.Message):
...
async def receive_clean_photo(msg: types.Message):
    ...
    async with aiosqlite.connect(db.DB_PATH) as conn:
        ...
        if worker:
            ...
            await conn.commit()
    worker_state[uid] = None
    await msg.answer("✅ Tozalash rasmi yuborildi.", reply_markup=worker_menu())
...
async def problem_start(msg: types.Message):
    worker_state[msg.from_user.id] = "waiting_problem"
...
async def receive_problem_photo(msg: types.Message):
    uid = msg.from_user.id
    if worker_state.get(uid) != "waiting_problem":
        return
    file_id = msg.photo[-1].file_id
    caption = msg.caption or ""
    async with aiosqlite.connect(db.DB_PATH) as conn:
...
async def today_reports(msg: types.Message):
    ...
    for r in reports:
        text += f"🆔 {r[0]} — {r[2]}\n{r[1]}\n\n"
    text += "✏️ Tahrirlash uchun 'tahrir <ID>' yozing."
...
async def show_finances(msg: types.Message):
    ...
    await msg.bot.send_message(SUPERADMIN_ID, f"💰 <b>{wname}</b> bonus/jarima ma’lumotini ko‘rdi.", parse_mode="HTML")
